# generate_structural_fault_k2_reduced.py
...
faulty_system_config["stiffness"] = list(original_system_config["stiffness"])
faulty_system_config["stiffness"][1] *= 0.5
print(f"Simulating structural fault: k[1] reduced by 50%. New stiffness: {faulty_system_config['stiffness']}")
...
# ----------------------------
def run_simulation(current_system_config, output_filename="structural_fault_k2_reduced.csv", normal_data_ref_file="vae_input_data.csv"):
    ...
    if plot_comparison:
        # Plot for a representative DOF, e.g., x2, as k[1] affects DOF1-DOF2 interaction
        # and DOF2 itself.
        dof_to_plot_idx = 1 # Index for x2 (0-based)
        channel_to_plot = f'x{dof_to_plot_idx+1}' # e.g., 'x2'

        plt.figure(figsize=(12, 6))
        plt.plot(t_eval, df_normal_ref[channel_to_plot], label=f'Normal - {channel_to_plot}', alpha=0.7)
        plt.plot(t_eval, df_vae_faulty[channel_to_plot], label=f'Faulty (k2 reduced) - {channel_to_plot}', linestyle='--')
        plt.title(f'Comparison: Normal vs. Structural Fault (k[1] reduced) - {channel_to_plot}')
        plt.xlabel('Time (s)')
...
if __name__ == "__main__":
    run_simulation(faulty_system_config,
                   output_filename="structural_fault_k2_reduced.csv",
