...
FAULT_FILE = "structural_fault_k2_reduced.csv"
...
def plot_dof2_normal_vs_fault_separated(normal_file, fault_file, dt=0.01):
    df_normal = pd.read_csv(normal_file)
    df_fault = pd.read_csv(fault_file)
    t = np.arange(len(df_normal)) * dt
    # Plot x2, v2, a2 (DOF 2 is column index 1)
    fig, axs = plt.subplots(3, 2, figsize=(14, 10), gridspec_kw={'width_ratios': [1, 1]}, sharex='col')
    axs[0,0].plot(t, df_normal["x2"], label="Normal x2", color='tab:blue', linewidth=1)
    axs[0,0].set_ylabel("Displacement (m)")
    axs[0,0].legend()
    axs[0,0].grid(True)
    axs[0,0].set_title("x2: Normal")
    axs[0,1].plot(t, df_fault["x2"], label="Fault x2 (k2 reduced)", color='red', linewidth=1)
    axs[0,1].set_ylabel("Displacement (m)")
    axs[0,1].legend()
    axs[0,1].grid(True)
    axs[0,1].set_title("x2: Fault")
    axs[1,0].plot(t, df_normal["v2"], label="Normal v2", color='tab:orange', linewidth=1)
    axs[1,0].set_ylabel("Velocity (m/s)")
    axs[1,0].legend()
    axs[1,0].grid(True)
    axs[1,0].set_title("v2: Normal")
    axs[1,1].plot(t, df_fault["v2"], label="Fault v2 (k2 reduced)", color='maroon', linewidth=1)
    axs[1,1].set_ylabel("Velocity (m/s)")
    axs[1,1].legend()
    axs[1,1].grid(True)
    axs[1,1].set_title("v2: Fault")
    axs[2,0].plot(t, df_normal["a2"], label="Normal a2", color='tab:green', linewidth=1)
    axs[2,0].set_ylabel("Acceleration (m/s²)")
    axs[2,0].legend()
    axs[2,0].grid(True)
    axs[2,0].set_title("a2: Normal")
    axs[2,0].set_xlabel("Time (s)")
    axs[2,1].plot(t, df_fault["a2"], label="Fault a2 (k2 reduced)", color='darkgreen', linewidth=1)
    axs[2,1].set_ylabel("Acceleration (m/s²)")
    axs[2,1].legend()
    axs[2,1].grid(True)
    axs[2,1].set_title("a2: Fault")
    axs[2,1].set_xlabel("Time (s)")
    plt.suptitle("DOF 2: Normal vs Fault (k2 Reduced by 50%) — Displacement, Velocity, Acceleration")
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.show()
    # Difference plots
    fig2, axs2 = plt.subplots(3, 1, figsize=(14, 8), sharex=True)
    axs2[0].plot(t, df_normal["x2"] - df_fault["x2"], color='purple', label='x2: Normal - Fault')
    axs2[0].set_ylabel("Δ Displacement (m)")
    axs2[0].set_title("Displacement Difference (x2)")
    axs2[0].legend()
    axs2[0].grid(True)
    axs2[1].plot(t, df_normal["v2"] - df_fault["v2"], color='brown', label='v2: Normal - Fault')
    axs2[1].set_ylabel("Δ Velocity (m/s)")
    axs2[1].set_title("Velocity Difference (v2)")
    axs2[1].legend()
    axs2[1].grid(True)
    axs2[2].plot(t, df_normal["a2"] - df_fault["a2"], color='darkgreen', label='a2: Normal - Fault')
    axs2[2].set_ylabel("Δ Acceleration (m/s²)")
    axs2[2].set_title("Acceleration Difference (a2)")
    axs2[2].set_xlabel("Time (s)")
    axs2[2].legend()
    axs2[2].grid(True)
    plt.suptitle("Difference (Normal - Fault) for DOF 2")
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.show()

if __name__ == "__main__":
    # --- Fault config: k2 reduced by 50% ---
    fault_config = {
        "mass": [BASE_MASS * 1.2, BASE_MASS, BASE_MASS, BASE_MASS * 0.8],
        "stiffness": [BASE_STIFFNESS * 1.5, BASE_STIFFNESS * 1.2 * 0.5, BASE_STIFFNESS, BASE_STIFFNESS * 0.8],
        "beta": 0.25,
        "gamma": 0.5,
        "num_dofs": 4
    }
    ...
    run_simulation(fault_config, force_tensor, FAULT_FILE, SIM_DURATION, dt=DT, zeta=DAMPING_RATIO)
    # Plot DOF 2 comparison
    plot_dof2_normal_vs_fault_separated(NORMAL_FILE, FAULT_FILE, dt=DT)
