# generate_sensor_fault_v3_noisy.py

import pandas as pd
import numpy as np
import matplotlib.pyplot as plt # Added for plotting

def apply_v3_noisy_fault(input_csv="vae_input_data.csv", output_csv="sensor_fault_v3_noisy.csv", noise_factor=1.5):
    try:
    ...
    print(f"Sensor fault data ({target_column} is noisy) saved to '{output_csv}'")

    # --- Plotting Comparison ---
    time_steps = np.arange(len(original_signal_normal))
    # (Similar time axis consideration as in x1_zero script)
    ...
    plt.title(f'Comparison: Normal vs. Sensor Fault ({target_column} Noisy)')
