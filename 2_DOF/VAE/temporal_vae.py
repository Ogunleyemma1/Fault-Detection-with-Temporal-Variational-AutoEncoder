...
class VAE(nn.Module):
    def __init__(self, input_dim=6, latent_dim=3):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(input_dim, 16),  # First hidden layer with 32 neurons
            nn.ReLU(),
            nn.Linear(16, 8),         # Second hidden layer with 16 neurons
            nn.ReLU(),
        )

        # Output layers from encoder to estimate parameters of latent Gaussian distribution
        self.fc_mu = nn.Linear(8, latent_dim)        # Mean of latent space
        self.fc_logvar = nn.Linear(8, latent_dim)    # Log-variance of latent space

        #  Decoder Network (Latent → Hidden → Reconstructed Input)
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, 8),  # Reverse path of encoder
            nn.ReLU(),
            nn.Linear(8, 16),
            nn.ReLU(),
