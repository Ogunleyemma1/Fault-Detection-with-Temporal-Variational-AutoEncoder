...
def test_vae():
    ...
    for i, label in enumerate(variable_labels):  # loop over x, v, a
        ...
        if num_dofs == 1:
            axs = [axs]  # Ensure subscriptable

        for dof in range(num_dofs):
            idx = i * num_dofs + dof
            axs[dof].plot(input_norm[:, idx][:timesteps], label='Normalized Input', color='tab:blue')
            axs[dof].plot(recon_np[:, idx][:timesteps], label='Normalized Recon', color='tab:orange', alpha=0.7)
