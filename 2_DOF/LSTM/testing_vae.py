...
def test_vae(seq_len=100):
    ...
    model = VAE(input_dim=6, latent_dim=16, hidden_dim=128, num_layers=2, dropout=0.3).to(device)
    ...
    # Overlap-averaged full reconstruction
    ...
    # Plotting by variable type (x, v, a)
    time = np.arange(len(data_norm))
    num_dofs = data.shape[1] // 3
    var_labels = ['x', 'v', 'a']
    colors = ['tab:blue', 'tab:orange']

    for i, var in enumerate(var_labels):  # x, v, a
