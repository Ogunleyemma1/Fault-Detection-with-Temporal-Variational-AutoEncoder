...
def train_vae(seq_len=80):
    print("Training Temporal VAE (Original + Drifted + Amplitude + High-Frequency)...")

    df = pd.read_csv("1DOF_signal_clean_variants.csv")
    df = df.loc[:int(0.5 * len(df))]  # use half the time series for training
    ...
    # === Plotting ===
    os.makedirs("VAE Training Plot", exist_ok=True)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("VAE Training Plot/loss_plot.png")
    plt.show()

    # === Latent Space Visualization ===
    print("🔍 Extracting latent space representations...")
    variants = {
        "Original": list(range(0, 3)),
        "Drifted": list(range(3, 6)),
        "Amplitude": list(range(6, 9)),
        "HighFreq": list(range(9, 12)),
    }
    ...
    for label, cols in variants.items():
        ...
        variant_data[:, cols] = data_norm[:, cols]
    ...
    colors = {"Original": "blue", "Drifted": "orange", "Amplitude": "purple", "HighFreq": "red"}
    ...
    plt.title("Latent Space PCA by Signal Type")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("VAE Training Plot/latent_space_pca.png")
    plt.show()
    ...
    plt.title("Latent Dimension Boxplot by Signal Type")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("VAE Training Plot/latent_boxplot.png")
    plt.show()

# === Run ===
if __name__ == "__main__":
    train_vae(seq_len=80)
